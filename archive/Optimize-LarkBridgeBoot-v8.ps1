...
if ([string]::IsNullOrWhiteSpace($LogPath)) {
    ...
    $LogPath = Join-Path (Get-Location) "LarkBridge-v8-$Mode-$stamp.txt"
}
...
$RemoteFile = "/tmp/larkbridge-v8-$PID.sh"
$TempFile = Join-Path ([System.IO.Path]::GetTempPath()) "larkbridge-v8-$PID.sh"
...
Write-Host ""
Write-Host "============================================================"
Write-Host " LarkBridge v8 post-checkpoint optimizer"
Write-Host "============================================================"
...
try {
    ...
    # Capture output as plain text while still displaying it.
    $output = & ssh.exe -tt $Target "TERM=dumb sudo bash '$RemoteFile'" 2>&1
