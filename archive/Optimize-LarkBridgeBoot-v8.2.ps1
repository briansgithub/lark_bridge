param(
    ...
    # OPTIONAL. Default is to preserve all admin SSH/GPG agent sockets for
    # future closed-loop automation and remote development. Incoming SSH is
    # never modified by this script.
    [switch]$TrimUserAgentSockets,
...
if ([string]::IsNullOrWhiteSpace($LogPath)) {
    ...
    $LogPath = Join-Path (Get-Location) "LarkBridge-v8.2-$Mode-$stamp.txt"
}
...
MODE="__MODE__"
TRIM_USER_AGENTS="__TRIM_USER_AGENTS__"
...
apply_user_agent_trim() {
    if [ "$TRIM_USER_AGENTS" != "yes" ]; then
        say "user agent sockets:"
        say "  preserved for future SSH/GPG/automation workflows (default)"
        return 0
    fi
...
audit() {
    ...
    say "remote_access_policy=preserve SSH/NetworkManager/admin agent sockets by default"
...
apply_all() {
    ...
    say "============================================================"
    say " Applying LarkBridge v8.2 recommended refinements"
    say "============================================================"
    say
    say "remote access:"
    say "  ssh.service and its configuration will not be modified"
    say "  NetworkManager architecture/profile will not be modified"
    if [ "$TRIM_USER_AGENTS" != "yes" ]; then
        say "  admin SSH/GPG agent sockets will be preserved"
    fi
    say
    ...
    say "No SSH, Bluetooth, NetworkManager, PipeWire, or WirePlumber service was restarted."
...
$RemoteScript = $RemoteScript.Replace(
    "__TRIM_USER_AGENTS__",
    $(if ($TrimUserAgentSockets) { "yes" } else { "no" })
)

# Avoid the CRLF-over-ssh problem encountered in earlier ad-hoc audits.
$RemoteScript = $RemoteScript -replace "`r`n", "`n"

$RemoteFile = "/tmp/larkbridge-v8.2-$PID.sh"
$TempFile = Join-Path ([System.IO.Path]::GetTempPath()) "larkbridge-v8.2-$PID.sh"
...
Write-Host ""
Write-Host "============================================================"
Write-Host " LarkBridge v8.2 post-checkpoint optimizer"
Write-Host "============================================================"
Write-Host "Target: $Target"
Write-Host "Mode:   $Mode"
Write-Host "Log:    $LogPath"
if ($TrimUserAgentSockets) {
    Write-Host "User agent sockets: TRIM IF UNUSED (explicit opt-in)"
}
else {
    Write-Host "User agent sockets: PRESERVE (default for closed-loop automation)"
}
Write-Host ""
...
try {
    ...
    # when -tt is used. Windows PowerShell 5.x wraps native stderr as an
    # ErrorRecord, which otherwise produces a misleading red stack trace.
    #
    # Keep the stream live, normalize native stderr ErrorRecords back to their
    # plain message text, and tee EVERYTHING to the requested TXT log.
