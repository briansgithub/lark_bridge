<#
.SYNOPSIS
    Safely analyzes and optimizes Raspberry Pi boot for LarkBridge (v6).
...
btfw_retry_audit() {
    ...
    if have_unit "$BTFW_SERVICE"; then
        printf 'service_active=%s service_failed=%s\n' \
            "$(systemctl is-active "$BTFW_SERVICE" 2>/dev/null || true)" \
            "$(systemctl is-failed "$BTFW_SERVICE" 2>/dev/null || true)"
    fi
...
# =====================================================================
# 7. Application service enablement
#
...
if [ -n "${SELECTED_SERVICE:-}" ]; then
    ...
    # =================================================================
    # 7. Application restart behavior
    #
    # EXPLICIT opt-in.
    #
    # Never:
